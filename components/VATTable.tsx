...
const Ico = {
  ...
  Percent:  () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="19" y1="5" x2="5" y2="19"/><circle cx="6.5" cy="6.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/></svg>,
};
      ...
      <AnimatePresence>
        {toasts.map(t => (
          ...
          >
            ...
            {t.type === "info"    && <span className="text-[#6C63FF] mr-1.5">·</span>}
            {t.message}
          </motion.div>
        ))}
      </AnimatePresence>
...
// ── Inline edit ────────────────────────────────────────────────────────────────
function InlineEdit({ row, vatRate, onSave, onCancel }: {
  row: PriceRow; vatRate: number;
  ...
  // live preview
  const gVal = parseFloat(gross.replace(/,/g, "")) || 0;
  const divisor = 1 + vatRate / 100;
  const previewNet = gVal > 0 ? gVal / divisor : 0;
  const previewVat = gVal - previewNet;
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <input value={label} onChange={e => setLabel(e.target.value)} onKeyDown={onKey}
        className={`${iCls} w-32`} placeholder="Label" />
      ...
      {gVal > 0 && (
        <span className="text-[10px] font-mono text-[#8E8EA0]">
          → <span style={{ color: "#6C63FF" }}>฿{formatTHB(previewNet)}</span>
          {" "}+<span style={{ color: "#FF6B35" }}> ฿{formatTHB(previewVat)}</span>
        </span>
      )}
...
function VatTooltip({ gross, vatRate }: { gross: number; vatRate: number }) {
  const divisor = 1 + vatRate / 100;
  ...
  return (
    <div className="absolute z-40 right-0 top-full mt-2 w-52 rounded-xl border border-[#E5E5EA] dark:border-[#2A2A38] bg-white dark:bg-[#1C1C26] shadow-2xl p-3.5">
      ...
      <div className="space-y-1.5 font-mono text-xs">
        <div className="flex justify-between"><span className="text-[#8E8EA0]">Incl. VAT</span><span className="font-medium text-[#111118] dark:text-[#F7F7F9]">฿{formatTHB(gross)}</span></div>
        <div className="flex justify-between"><span className="text-[#8E8EA0]">Before VAT</span><span className="font-semibold" style={{ color: "#6C63FF" }}>฿{formatTHB(net)}</span></div>
        <div className="flex justify-between"><span className="text-[#8E8EA0]">VAT ({vatRate.toFixed(2)}%)</span><span style={{ color: "#FF6B35" }}>฿{formatTHB(vat)}</span></div>
        <div className="border-t border-[#E5E5EA] dark:border-[#2A2A38] pt-1.5 flex justify-between"><span className="text-[#8E8EA0]">Divisor</span><span className="text-[#111118] dark:text-[#F7F7F9]">{divisor.toFixed(4)}</span></div>
        <div className="flex justify-between"><span className="text-[#8E8EA0]">VAT share</span><span className="text-[#111118] dark:text-[#F7F7F9]">{(vat / gross * 100).toFixed(2)}%</span></div>
      </div>
      {/* Mini bar */}
      <div className="mt-3">
        <div className="flex rounded-full overflow-hidden h-1.5">
          <div style={{ width: `${(net / gross * 100).toFixed(1)}%`, background: "#6C63FF" }} />
          <div style={{ width: `${(vat / gross * 100).toFixed(1)}%`, background: "#FF6B35" }} />
        </div>
        <div className="flex justify-between mt-1">
          <span className="text-[9px] font-mono" style={{ color: "#6C63FF" }}>Net {(net/gross*100).toFixed(1)}%</span>
          <span className="text-[9px] font-mono" style={{ color: "#FF6B35" }}>VAT {(vat/gross*100).toFixed(1)}%</span>
        </div>
      </div>
    </div>
  );
...
// ── VAT Rate input ─────────────────────────────────────────────────────────────
function VatRateInput({ vatRate, onChange }: { vatRate: number; onChange: (v: number) => void }) {
  const [draft, setDraft] = useState(String(vatRate));
  const [focused, setFocused] = useState(false);

  const commit = () => {
    const v = parseFloat(draft);
    if (!isNaN(v) && v >= 0 && v <= 100) onChange(parseFloat(v.toFixed(4)));
    else setDraft(String(vatRate));
    setFocused(false);
  };

  useEffect(() => {
    if (!focused) setDraft(String(vatRate));
  }, [vatRate, focused]);

  const divisor = 1 + vatRate / 100;
  const presets = [7, 10, 15, 20.84];

  return (
    <div className="relative group">
      <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl border border-[#E5E5EA] dark:border-[#2A2A38] bg-white dark:bg-[#1C1C26] transition-colors focus-within:border-[#6C63FF]">
        <Ico.Percent />
        <span className="text-[10px] font-mono text-[#8E8EA0] tracking-widest uppercase">VAT</span>
        <input
          type="number"
          value={focused ? draft : vatRate}
          onChange={e => setDraft(e.target.value)}
          onFocus={() => { setFocused(true); setDraft(String(vatRate)); }}
          onBlur={commit}
          onKeyDown={e => { if (e.key === "Enter") commit(); if (e.key === "Escape") { setDraft(String(vatRate)); setFocused(false); } }}
          min={0} max={100} step={0.01}
          className="w-16 font-mono text-sm font-semibold text-[#111118] dark:text-[#F7F7F9] bg-transparent outline-none text-right [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
        />
        <span className="text-sm font-mono text-[#8E8EA0]">%</span>
      </div>
      {/* Preset dropdown on hover */}
      <div className="absolute top-full left-0 mt-1.5 w-52 rounded-xl border border-[#E5E5EA] dark:border-[#2A2A38] bg-white dark:bg-[#1C1C26] shadow-xl p-2 z-30 hidden group-hover:block">
        <p className="text-[9px] font-mono text-[#8E8EA0] uppercase tracking-widest px-2 pb-1.5">Common rates</p>
        {presets.map(p => (
          <button key={p} onClick={() => { onChange(p); setDraft(String(p)); }}
            className={`w-full text-left px-2 py-1.5 rounded-lg text-xs font-mono flex justify-between items-center transition-colors ${vatRate === p ? "bg-[#EEEDFF] dark:bg-[#2A2845] text-[#6C63FF]" : "text-[#111118] dark:text-[#F7F7F9] hover:bg-[#F7F7F9] dark:hover:bg-[#16161F]"}`}>
            <span>{p}%</span>
            <span className="text-[#8E8EA0]">÷ {(1 + p / 100).toFixed(4)}</span>
          </button>
        ))}
        <div className="border-t border-[#E5E5EA] dark:border-[#2A2A38] mt-1.5 pt-1.5 px-2">
          <p className="text-[9px] font-mono text-[#8E8EA0]">Current divisor: <span className="text-[#111118] dark:text-[#F7F7F9]">{divisor.toFixed(4)}</span></p>
        </div>
      </div>
    </div>
  );
...
function useHistory<T>(initial: T) {
  ...
  const undo = useCallback(() => {
    ...
    if (prev !== undefined) {
      setState(curr => { future.current.push(curr); return prev; });
      return true;
    }
    return false;
  }, []);
...
export default function VATTable() {
  const { state: rows, set: setRows, undo: undoRows, canUndo } = useHistory<PriceRow[]>(generateDefaultRows(DEFAULT_VAT_RATE));
  const [vatRate, setVatRateState]     = useState(DEFAULT_VAT_RATE);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === "z" && !e.shiftKey) {
        e.preventDefault();
        const ok = undoRows();
        if (ok) push("Undo", "info");
        else push("Nothing to undo", "error");
      }
    };
  ...
  const flash = (id: string) => { setFlashId(id); setTimeout(() => setFlashId(null), 1400); };

  // Recalculate all rows when VAT rate changes
  const setVatRate = useCallback((rate: number) => {
    setVatRateState(rate);
    setRows(prev => prev.map(r => calcRow(r.grossPrice, r.label, r.id, rate, r.isCustom, r.pinned)));
    push(`VAT rate → ${rate}%  (÷ ${(1 + rate / 100).toFixed(4)})`);
  }, [setRows, push]);

  // Format numbers with chosen decimal precision
  const fmt = useCallback((v: number) => formatTHB(v, decimals), [decimals]);
  ...
  const displayPinned = applySort(pinned);
  const displayUnpinned = applySort(unpinned);
  const displayRows = [...displayPinned, ...displayUnpinned];
  ...
  const addRow = useCallback(() => {
    ...
    setRows(p => [...p, calcRow(val, label, id, vatRate, true)]);
    setCustomAmt(""); setCustomLabel("");
    setTimeout(() => flash(id), 20);
    push("Row added");
  }, [customAmt, customLabel, vatRate, setRows, push]);

  const duplicateRow = useCallback((row: PriceRow) => {
    const id = `custom-${++_cid}`;
    const newRow = calcRow(row.grossPrice, `${row.label} (copy)`, id, vatRate, true);
    setRows(p => {
    ...
    setTimeout(() => flash(id), 20);
    push("Row duplicated");
  }, [vatRate, setRows, push]);

  const saveEdit = useCallback((id: string, gross: number, label: string) => {
    setRows(p => p.map(r => r.id === id ? calcRow(gross, label, id, vatRate, r.isCustom, r.pinned) : r));
    setEditingId(null);
    setTimeout(() => flash(id), 20);
    push("Row updated");
  }, [vatRate, setRows, push]);
  ...
  const togglePin = useCallback((id: string) => {
    setRows(p => p.map(r => r.id === id ? { ...r, pinned: !r.pinned } : r));
    const row = rows.find(r => r.id === id);
    push(row?.pinned ? "Unpinned" : "Pinned to top", "info");
  }, [rows, setRows, push]);

  const copyRow = useCallback((row: PriceRow) => {
    const text = [row.label, `฿${fmt(row.grossPrice)}`, `฿${fmt(row.netPrice)}`, `฿${fmt(row.vatAmount)}`].join("\t");
    navigator.clipboard.writeText(text).then(() => push("Row copied"));
  }, [fmt, push]);

  const copyAll = useCallback(() => {
    const header = `Label\tIncl. VAT\tBefore VAT\tVAT Amount\tVAT Rate`;
    const body = rows.map(r =>
      `${r.label}\t฿${fmt(r.grossPrice)}\t฿${fmt(r.netPrice)}\t฿${fmt(r.vatAmount)}\t${vatRate}%`
    ).join("\n");
    navigator.clipboard.writeText(`${header}\n${body}`).then(() => push(`${rows.length} rows copied`));
  }, [rows, vatRate, fmt, push]);

  const exportCSV = useCallback(() => {
    const header = `Label,Price incl. VAT (฿),Before VAT (฿),VAT Amount (฿),VAT Rate (%)`;
    const body = rows.map(r =>
      `"${r.label}",${r.grossPrice.toFixed(decimals)},${r.netPrice.toFixed(decimals)},${r.vatAmount.toFixed(decimals)},${vatRate}`
    ).join("\n");
    const blob = new Blob([`${header}\n${body}`], { type: "text/csv;charset=utf-8;" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `vat-${vatRate}pct.csv`; a.click();
    URL.revokeObjectURL(a.href);
    push("CSV exported");
  }, [rows, vatRate, decimals, push]);

  const exportExcel = useCallback(async () => {
    try {
      const XLSX = await import("xlsx");
      const data = [
        ["Label", "Price incl. VAT (฿)", "Before VAT (฿)", "VAT Amount (฿)", "VAT Rate (%)"],
        ...rows.map(r => [r.label, r.grossPrice, r.netPrice, r.vatAmount, vatRate]),
      ];
      const ws = XLSX.utils.aoa_to_sheet(data);
      ws["!cols"] = [{ wch: 24 }, { wch: 20 }, { wch: 18 }, { wch: 16 }, { wch: 12 }];
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "VAT Prices");
      XLSX.writeFile(wb, `vat-${vatRate}pct.xlsx`);
      push("Excel exported");
  ...
  const reset = useCallback(() => {
    setRows(generateDefaultRows(DEFAULT_VAT_RATE, step));
    setVatRateState(DEFAULT_VAT_RATE);
    setSearch(""); setSortCol(null); setEditingId(null);
  ...
  const applyStep = useCallback((newStep: number) => {
    ...
    setRows(p => {
      ...
      for (let price = 15000; price <= 18000; price += newStep)
        presets.push(calcRow(price, `฿${formatTHB(price, 0)}`, `preset-${price}-${newStep}`, vatRate, false));
      return [...presets, ...customs];
    });
    push(`Step → ฿${newStep.toLocaleString()}`);
  }, [vatRate, setRows, push]);
  ...
  const W = { label: "flex-1 min-w-0", gross: "w-36 shrink-0", net: "w-36 shrink-0", vat: "w-32 shrink-0", bar: "w-20 shrink-0", actions: "w-28 shrink-0" };
  const thCls = "text-[10px] font-mono tracking-widest text-[#8E8EA0] uppercase cursor-pointer select-none hover:text-[#111118] dark:hover:text-[#F7F7F9] transition-colors text-right bg-transparent border-none p-0";

  // divisor shown in header
  const divisor = (1 + vatRate / 100).toFixed(4);
      ...
      <div className="min-h-screen bg-[#F7F7F9] dark:bg-[#111118] transition-colors duration-300 px-4 py-8 md:px-10">
        ...
        <div className="max-w-5xl mx-auto mb-6 no-print">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {/* VAT rate input — prominent */}
              <VatRateInput vatRate={vatRate} onChange={setVatRate} />

              <div className="w-px h-6 bg-[#E5E5EA] dark:bg-[#2A2A38]" />

              <button onClick={() => { const ok = undoRows(); if (ok) push("Undo", "info"); else push("Nothing to undo", "error"); }}
                className={`${ghostBtn} ${canUndo ? "" : "opacity-40"}`} title="Undo (Ctrl+Z)">
                <Ico.Undo />Undo
              </button>
              ...
              <button onClick={handlePrint} className={ghostBtn}><Ico.Print />Print</button>
          ...
          <div className="flex gap-3 mt-4 flex-wrap items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              {[250, 500, 1000].map(s => (
                <button key={s} onClick={() => applyStep(s)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-mono border transition-all ${step === s ? "bg-[#6C63FF] text-white border-[#6C63FF]" : "border-[#E5E5EA] dark:border-[#2A2A38] text-[#8E8EA0] hover:border-[#6C63FF] hover:text-[#6C63FF]"}`}>
                  ฿{s.toLocaleString()}
                </button>
              ))}
            </div>
            ...
            {/* Decimal precision */}
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono tracking-widest text-[#8E8EA0] uppercase">Decimals</span>
              {[0, 2, 4].map(d => (
                <button key={d} onClick={() => { setDecimals(d); push(`${d} decimal places`); }}
                  className={`px-2.5 py-1.5 rounded-lg text-xs font-mono border transition-all ${decimals === d ? "bg-[#111118] dark:bg-[#F7F7F9] text-white dark:text-[#111118] border-[#111118] dark:border-[#F7F7F9]" : "border-[#E5E5EA] dark:border-[#2A2A38] text-[#8E8EA0] hover:border-[#111118] dark:hover:border-[#F7F7F9]"}`}>
                  {d}
                </button>
              ))}
            </div>
        ...
        <div className="max-w-5xl mx-auto print-area">
          ...
          <div className="hidden print:block mb-4">
            <h2 className="text-lg font-semibold">VAT Price Table — {vatRate}% VAT (÷ {divisor})</h2>
          ...
          <div className="rounded-2xl border border-[#E5E5EA] dark:border-[#2A2A38] overflow-hidden bg-white dark:bg-[#1C1C26] shadow-sm">
            ...
            <div className="flex items-center px-6 py-3 border-b border-[#E5E5EA] dark:border-[#2A2A38] bg-[#F7F7F9] dark:bg-[#16161F] gap-4">
              <button onClick={() => handleSort("label")} className={`${W.label} ${thCls} text-left`}>
                Label <SortArrow col="label" />
              </button>
              <button onClick={() => handleSort("gross")} className={`${W.gross} ${thCls}`}>
                Incl. VAT <SortArrow col="gross" />
              </button>
              <button onClick={() => handleSort("net")} className={`${W.net} ${thCls}`}>
                Before VAT <SortArrow col="net" />
              </button>
              <button onClick={() => handleSort("vat")} className={`${W.vat} ${thCls}`}>
                VAT ({vatRate}%) <SortArrow col="vat" />
              </button>
              <div className={`${W.bar} text-[10px] font-mono tracking-widest text-[#8E8EA0] uppercase text-right`}>Split</div>
              <div className={`${W.actions} no-print`} />
            </div>

            {/* Rows */}
            <AnimatePresence initial={false}>
              {displayRows.length === 0 ? (
                <div className="py-14 text-center text-sm text-[#8E8EA0] font-sans">No rows match your filter.</div>
              ) : displayRows.map((row, i) => {
                const barNet = (row.netPrice / row.grossPrice * 100);
                const barVat = (row.vatAmount / row.grossPrice * 100);
                return (
                  <motion.div
                    key={row.id}
                    layout
                    ...
                    className={`
                      ...
                      ${row.pinned ? "bg-[#FFFBEB] dark:bg-[#1C1A0A]" : ""}
                      ${flashId === row.id ? "bg-[#EEEDFF] dark:bg-[#2A2845]" : (!row.pinned ? "hover:bg-[#F7F7F9] dark:hover:bg-[#16161F]" : "")}
                    `}
                  >
                    {/* Row number */}
                    ...
                    <div className={`${W.label} min-w-0`}>
                      {editingId === row.id ? (
                        <InlineEdit row={row} vatRate={vatRate}
                          onSave={(g, l) => saveEdit(row.id, g, l)}
                          onCancel={() => setEditingId(null)} />
                      ) : (
                        <div className="flex items-center gap-2 min-w-0">
                          {row.pinned && <span className="shrink-0 text-amber-500"><Ico.Pin /></span>}
                          {row.isCustom && (
                            <span className="shrink-0 text-[9px] font-mono px-1.5 py-0.5 rounded-full bg-[#EEEDFF] dark:bg-[#2A2845] text-[#6C63FF] leading-none">
                              custom
                            </span>
                          )}
                          <span className="text-sm font-sans text-[#111118] dark:text-[#F7F7F9] truncate">{row.label}</span>
                        </div>
                      )}
                    </div>
                    ...
                    {/* Mini split bar */}
                    <div className={`${W.bar} flex flex-col gap-0.5`}>
                      <div className="flex rounded-full overflow-hidden h-1.5 w-full">
                        <div style={{ width: `${barNet.toFixed(1)}%`, background: "#6C63FF" }} />
                        <div style={{ width: `${barVat.toFixed(1)}%`, background: "#FF6B35" }} />
                      </div>
                      <span className="text-[9px] font-mono text-[#8E8EA0] text-right">{barVat.toFixed(1)}%</span>
                    </div>

                    {/* Actions */}
                    <div className={`${W.actions} flex items-center justify-end gap-0.5 shrink-0 no-print`}>
                      {editingId !== row.id && (
                        <>
                          <div className="relative">
                            <button onMouseEnter={() => setTooltipId(row.id)} onMouseLeave={() => setTooltipId(null)}
                            ...
                            {tooltipId === row.id && <VatTooltip gross={row.grossPrice} vatRate={vatRate} />}
                          </div>
                          <button onClick={() => togglePin(row.id)}
                            className={`p-1.5 rounded-lg transition-colors opacity-0 group-hover:opacity-100 ${row.pinned ? "!opacity-100 text-amber-500 bg-amber-50 dark:bg-amber-950/30" : "text-[#8E8EA0] hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-950/30"}`}
                            title={row.pinned ? "Unpin" : "Pin to top"}>
                            <Ico.Pin />
                          </button>
                          <button onClick={() => copyRow(row)}
                            className="p-1.5 rounded-lg text-[#8E8EA0] hover:text-[#111118] dark:hover:text-[#F7F7F9] hover:bg-[#F0F0F5] dark:hover:bg-[#2A2A38] transition-colors opacity-0 group-hover:opacity-100"
                            title="Copy row"><Ico.Copy /></button>
                          <button onClick={() => duplicateRow(row)}
                            ...
                            title="Duplicate row"><Ico.Dup /></button>
                          <button onClick={() => setEditingId(row.id)}
                            className="p-1.5 rounded-lg text-[#8E8EA0] hover:text-[#6C63FF] hover:bg-[#EEEDFF] dark:hover:bg-[#2A2845] transition-colors opacity-0 group-hover:opacity-100"
                            title="Edit row"><Ico.Edit /></button>
                          <button onClick={() => deleteRow(row.id)}
                            ...
                            title="Delete row"><Ico.Trash /></button>
                        </>
                      )}
                    </div>
                  </motion.div>
            ...
            <div className="flex items-center justify-between px-6 py-2.5 bg-[#F7F7F9] dark:bg-[#16161F] border-t border-[#E5E5EA] dark:border-[#2A2A38]">
              <span className="text-[10px] font-mono text-[#8E8EA0]">
                {totalRows} of {rows.length} rows
                {pinned.length > 0 && ` · ${pinned.length} pinned`}
                {search ? ` · "${search}"` : ""}
              </span>
              <span className="text-[10px] font-mono text-[#8E8EA0]">
                VAT {vatRate}% · ÷ {divisor} · <span style={{ color: "#6C63FF" }}>■</span> Net · <span style={{ color: "#FF6B35" }}>■</span> VAT
              </span>
            </div>
          </div>

          {/* ── Add row panel ── */}
          <div className="mt-4 rounded-2xl border border-[#E5E5EA] dark:border-[#2A2A38] bg-white dark:bg-[#1C1C26] p-5 shadow-sm no-print">
            <p className="text-[10px] font-mono tracking-widest text-[#8E8EA0] uppercase mb-4">Add custom amount</p>
            <div className="flex flex-wrap gap-3 items-end">
              <div className="flex flex-col gap-1.5 flex-1 min-w-[140px]">
                <label className="text-xs font-sans text-[#8E8EA0]">Label <span className="opacity-50">(optional)</span></label>
              ...
              {customAmt && !isNaN(parseFloat(customAmt)) && parseFloat(customAmt) > 0 && (
                <div className="flex flex-col gap-1 text-xs font-mono self-end pb-2.5">
                  <span className="text-[#8E8EA0]">Preview @ {vatRate}%</span>
                  <span style={{ color: "#6C63FF" }}>฿{fmt(parseFloat(customAmt) / (1 + vatRate / 100))} net</span>
                  <span style={{ color: "#FF6B35" }}>฿{fmt(parseFloat(customAmt) - parseFloat(customAmt) / (1 + vatRate / 100))} VAT</span>
                </div>
              )}
          ...
          {/* ── Keyboard hints ── */}
          <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1.5 justify-center no-print">
            {[
              ["Enter", "Add row"],
              ["Ctrl+Z", "Undo"],
              ["Hover row", "Info · Pin · Copy · Dup · Edit · Delete"],
              ["VAT %", "Hover for common presets"],
              ["Decimals", "0 / 2 / 4 decimal places"],
            ].map(([k, d]) => (
          ...
          <p className="text-center text-[9px] font-mono text-[#8E8EA0] mt-4 mb-1 no-print">
            Before VAT = Incl. VAT ÷ {divisor} · VAT = Incl. VAT − Before VAT · Rate = {vatRate}%
          </p>
        </div>
      </div>
    </>
